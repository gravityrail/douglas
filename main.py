...
def create_crewai_setup(
    movie_slug, movie_name, movie_genre="Action", movie_idea="A heist movie"
):
...
"""
    ...
    image_generator_tool = Tool(
        ...
        DallEAPIWrapper(size="1792x1024", quality="hd", model="dall-e-3").run,
    ...
    write_treatment = Task(
        ...
        agent=screenwriter,
        output_file=f"{movie_dir}/treatment_draft.md",
        context=[define_plot],
    )
    ...
    def write_script_act(movie_name, movie_dir, act_description, act_file, agent, context=[]):
        return Task(
            description=f"""Write the {act_description} script for {movie_name} based on the treatment in the file {movie_dir}/treatment_final.md, in the {movie_genre} genre. Consult with the screenwriter, script consultant, and director to integrate any feedback.""",
    ...
    write_second_act = write_script_act(
        ...
        "second_act_draft.md",
        screenwriter,
        context=[treatment_final, write_first_act],
    )
    ...
    write_third_act = write_script_act(
        ...
        "third_act_draft.md",
        screenwriter,
        context=[treatment_final, write_first_act, write_second_act],
    )
    ...
    def illustrate_storyboard_task(movie_name, movie_dir, act_description, storyboard_file, illustration_file, context=[]):
        return Task(
            ...
            Also consult the overview of the movie in idea_draft.md.
            ...
            expected_output=f"Storyboard for the {act_description} of the movie in markdown format with shot title, description and embedded image. Do NOT wrap it in a code block.",
            agent=director,
    ...
    illustrate_first_act_storyboard = illustrate_storyboard_task(
        ...
        context=[treatment_final, storyboard_first_act],
    )
    ...
    illustrate_second_act_storyboard = illustrate_storyboard_task(
        ...
        context=[treatment_final, storyboard_second_act],
    )
    ...
    illustrate_third_act_storyboard = illustrate_storyboard_task(
        ...
        context=[treatment_final, storyboard_third_act],
    )
        ...
        with st.expander("Processing!"):
            ...
            with st.spinner("Generating Results"):
                crew_result = create_crewai_setup(
                    movie_slug, movie_name, movie_genre, movie_idea
                )
